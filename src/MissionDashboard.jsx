...
const NAT_OVERALL=7.1,NAT_PERSISTENT=19.4,DATASET_ID="7588c2d6-9e8a-4d84-8f19-6b8d52a01fbd";
function AttendanceTab(){
  ...
  useEffect(()=>{
    ...
    try{
      ...
      if(!oInd)throw new Error("Cannot find absence indicators");
      ...
      const matched=TARGET_LAS.map(la=>{const opt=laOpts.find(o=>o.oldCode===la.code||o.label?.toLowerCase()===la.name.toLowerCase()||o.label?.toLowerCase().includes(la.name.toLowerCase().split(" ")[0]));return opt?{...la,locId:opt.id}:null;}).filter(Boolean);
      ...
      const qr=await fetch(`https://api.education.gov.uk/statistics/v1/data-sets/${DATASET_ID}/query`,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({criteria:{geographicLevels:{comparator:"In",value:["LA","NAT"]},locations:{comparator:"In",value:locIds}},indicators:indIds,sort:[{field:"timePeriod",order:"Desc"}],page:1,pageSize:100})});
      if(!qr.ok)throw new Error("Query "+qr.status);
